...
from data_loader import FADADataset, FADADatasetIndividual
import models
import data_loader
...
datasets_super = {x: datasets.ImageFolder(os.path.join(datadir_super, src, x),
                                          data_transforms[x])
                  for x in ['train']}
...
dataloaders_1 = {x: torch.utils.data.DataLoader(datasets_1[x], batch_size=60,
              ...
              for x in ['train', 'val']}
base_mapping = datasets_super['train'].class_to_idx
...
ind_dir = '/home/villacis/Desktop/villacis/datasets/pc20_indpairs_mini'
siamese_dataset = FADADatasetIndividual(os.path.join(datadir, src),
                                       os.path.join(datadir, dst),
                                       os.path.join(ind_dir, src),
                                       os.path.join(ind_dir, dst),
                                        'train',
                                        base_mapping,
                                        img_size
                                        )
