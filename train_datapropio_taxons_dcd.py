...
from data_loader import FADADatasetTaxonsDCD
import models
import data_loader
...
data_dir2 = '/media/goeau/DATA/villacis/datasets/todas/todo_split'
#data_dir2 = '/media/goeau/DATA/villacis/datasets/todas/todo_split'
#data_dir2 = '/home/villacis/todo_split/train'
#data_dir2 = '/media/goeau/DATA/villacis/datasets/plantclef20_split/photo'
...
correct = 0.0
total = 0.0
encoder.load_state_dict(torch.load('result/encoder_fada.pth'))
classifier.load_state_dict(torch.load('result/classifier_fada.pth'))
#classifier.load()
for data, labels in dataloaders_1['val']:
    with torch.no_grad():
        data=data.to(device)
...
siamese_dataset = FADADatasetTaxonsDCD(os.path.join(datadir, src),
                                       data_dir2,
                                        'train',
                                        base_mapping,
                                        class_name_to_id,
...
print("||||| Stage 3 |||||")
optimizer_g_h=torch.optim.Adam(list(encoder.parameters())+list(classifier.parameters()),lr=0.0001)  #0.0001 0.0001=0.11
optimizer_d=torch.optim.Adam(list(discriminator.parameters())+list(discriminator_genus.parameters()),lr=0.0001) #0.0001
scheduler_g_h = lr_scheduler.MultiStepLR(optimizer_g_h, milestones=[10], gamma=0.1)
scheduler_d = lr_scheduler.MultiStepLR(optimizer_d, milestones=[10], gamma=0.1)
...
for epoch in range(num_epochs3):
    ...
    discriminator.train()
    confusion_matrix = torch.zeros(4, 4)
    for X1, X2, ground_truths_y1, ground_truths_y2, ground_truths, domains1, domains2, same, genus1, genus2, gt_genus, gt_family in dataloader_2:
        X1=X1.to(device)
        X2=X2.to(device)
        ...
        loss_dcd=0.5*(loss_fn(y_pred_dcd,dcd_labels) + loss_fn(y_pred_dcd_genus,dcd_labels_genus))
        ...
        loss_sum = loss_X1 + loss_X2 - 0.9*loss_dcd + 0.3*loss_genus
    ...
    for X1, X2, ground_truths_y1, ground_truths_y2, ground_truths, domains1, domains2, same, genus1, genus2, gt_genus, gt_family in dataloader_2:
        ...
        X_cat = torch.cat([encoder(X1), encoder(X2)], 1)
        y_pred = discriminator(X_cat.detach())
        y_pred_genus = discriminator(X_cat.detach())
        loss = loss_fn(y_pred, ground_truths)
        loss_genus =  loss_fn(y_pred_genus, gt_genus)
        loss_2 += loss.item()
        loss = loss + loss_genus
        loss.backward()
        optimizer_d.step()
    ...
    if(accuracy>best_acc):
        best_acc = accuracy
        torch.save(encoder.state_dict(),'result/encoder_taxons_extra.pth')
        #classifier.save()
        torch.save(classifier.state_dict(),'result/classifier_taxons_extra.pth')
