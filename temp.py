...
trainset=torchvision.datasets.ImageFolder("/home/villacis/Desktop/villacis/datasets/plantclef_superminida_17_20_split/herbarium/train", transform=transform, target_transform=None)
trainloader = torch.utils.data.DataLoader(trainset, shuffle = True , batch_size = batch_size , num_workers = 2)
testset=torchvision.datasets.ImageFolder("/home/villacis/Desktop/villacis/datasets/plantclef_superminida_17_20_split/herbarium/val", transform=transform, target_transform=None)
...
for epoch in range(iterations):
	...
	for i,data in enumerate(trainloader):
		...
		decoder_path = os.path.join('basura/', 'decoder-%d.pkl' %(epoch+1))
		encoder_path = os.path.join('basura/', 'encoder-%d.pkl' %(epoch+1))
		autoencoder_path = os.path.join('basura/', 'autoencoder-%d.pkl' %(epoch+1))
		classifier_path = os.path.join('basura/', 'classifier-%d.pkl' %(epoch+1))
		classification_path = os.path.join('basura/','classification-%d.pkl' %(epoch+1))
	...
	if ( epoch+1 )% 1 == 0:
		...
		for t_i,t_data in enumerate(testloader):
			...
			if (epoch + 1)%1 == 0:
				print("saving image")
				test_result_path = os.path.join('basura/', 'batch_%d_%d'%((epoch+1)/1,t_i+1) + '.jpg')
