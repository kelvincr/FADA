...
correct = 0.0
total = 0.0
encoder.load_state_dict(torch.load('result/encoder_fullextra_ss.pth'))
classifier.load_state_dict(torch.load('result/classifier_fullextra_ss.pth'))
#classifier.load()
...
optimizer_g_h=torch.optim.Adam(list(encoder.parameters())+list(classifier.parameters())+list(ssnet.parameters()),lr=0.0001)
optimizer_d=torch.optim.Adam(discriminator.parameters(),lr=0.0001) #0.001
scheduler_g_h = lr_scheduler.MultiStepLR(optimizer_g_h, milestones=[2], gamma=0.1)
scheduler_d = lr_scheduler.MultiStepLR(optimizer_d, milestones=[2], gamma=0.1)
...
for epoch in range(num_epochs3):
    ...
    if(accuracy>best_acc):
        best_acc = accuracy
        torch.save(encoder.state_dict(),'result/encoder_fullextra_ss_adicional.pth')
        #classifier.save()
        torch.save(classifier.state_dict(),'result/classifier_fullextra_ss_adicional.pth')
