...
dst = 'photo'       #svhn
img_size = 224  #32
#datadir = '/home/j/Documents/educacion/2020/1sem/practica/a4-code-v2-updated/digits'
...
#encoder = models.EncoderPro()
#encoder = tmodels.resnet50(pretrained=True)
#encoder.fc = nn.Sequential()
encoder = tmodels.inception_v3(pretrained=False)
...
datasets_1 = {x: datasets.ImageFolder(os.path.join(datadir, src, x),
                                          data_transforms[x])
                  for x in ['train', 'val']}
dataloaders_1 = {x: torch.utils.data.DataLoader(datasets_1[x], batch_size=40,
...
#encoder.save()
torch.save(encoder.state_dict(),'result/encoderpro.pth')

classifier.save()

#encoder.load()
encoder.load_state_dict(torch.load('result/encoderpro.pth'))
classifier.load()
...
for epoch in range(num_epochs2):
    ...
    for index in range(n_iters):
        ...
        if (index+1)%mini_batch_size==0:
            ...
            acc = (torch.max(y_test_pred,1)[1]==ground_truths).float().mean().item()
            
            loss=loss_fn(y_pred,ground_truths)
            loss.backward()
            optimizer_D.step()
            loss_mean.append(loss.item())
            acc_mean.aaappend(acc)
            X1 = []
            X2 = []
            ground_truths = []

    print("Phase 2 ---- Epoch %d/%d loss:%.3f acc:%.3f"%(epoch+1,num_epochs2,np.mean(loss_mean), np.mean(acc_mean)))
...
scheduler_g_h = lr_scheduler.MultiStepLR(optimizer_g_h, milestones=[50], gamma=0.1)
scheduler_d = lr_scheduler.MultiStepLR(optimizer_d, milestones=[50], gamma=0.1)

datasets_3 = {x: datasets.ImageFolder(os.path.join(datadir, dst, x),
                                          data_transforms[x])
                  for x in ['val']}
test_dataloader = torch.utils.data.DataLoader(datasets_3['val'], batch_size=128,
                                             shuffle=True, num_workers=6)            
...
groups, groups_y = data_loader.sample_groups(X_s,Y_s,X_t,Y_t,seed=110)
G1, G2, G3, G4 = groups
...
for epoch in range(num_epochs3):
    ...
    print("step3----Epoch %d/%d  accuracy: %.3f " % (epoch + 1, num_epochs3, accuracy))
