...
from data_loader import FADADatasetTaxons
import models
import data_loader
...
siamese_dataset = FADADatasetTaxons(os.path.join(datadir, src),
                                       data_dir2,
                                        'train',
                                        base_mapping,
                                        class_name_to_id,
...
optimizer_d=torch.optim.Adam(discriminator.parameters(),lr=0.0001) #0.0001
...
for epoch in range(num_epochs3):
    ...
    discriminator.train()
    confusion_matrix = torch.zeros(4, 4)
    for X1, X2, ground_truths_y1, ground_truths_y2, ground_truths, domains1, domains2, same, genus1, genus2 in dataloader_2:
        X1=X1.to(device)
        X2=X2.to(device)
